...
namespace arkanoid
{
    class TheBoard
    {
        ...
        {
            if (p.Y >= Canvas.GetTop(img))  // if the ball hits the top and bottom of the canvas
            {
                if (p.X > Canvas.GetLeft(img) && p.X < Canvas.GetLeft(img) + img.ActualWidth) //this if statement to know when to bounce off the board
                {     
                    return true;
                }
            }
